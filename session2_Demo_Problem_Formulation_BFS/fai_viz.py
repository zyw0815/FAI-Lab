...
def show_romania_map(highlight_path=None, show_sld=False,
    ...
    if show_sld:
        ...
        ax_sld.spines[['top', 'right']].set_visible(False)
...
def plot_bfs_dfs_exploration(problem, get_actions_fn,
    ...
    problem        : dict with 'initial', 'goal', and 'grid_size' (or 'rows'/'cols')
                     and optional 'walls'
    get_actions_fn : callable(problem, state) -> [(action, next_state), ...]
    """
    rows = cols = problem.get('grid_size', 5)
    rows = problem.get('rows', rows)
    cols = problem.get('cols', cols)
    ...
    def _bfs_order():
        ...
        while frontier:
            ...
            for _, ns in get_actions_fn(problem, state):
...
def plot_romania_algorithms(results, suptitle="Romania Road Map \u2014 Algorithm Comparison",
